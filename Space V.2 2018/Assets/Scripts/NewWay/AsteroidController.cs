...
public class AsteroidControlelr : MonoBehaviour
{
    ...
    void RunLoadReas()
    {
        if(HasRunLoad == false)
        {
            foreach(object obj in CF3.GetReas(Filepath))
            {
                AsteroidModels.Add(obj as GameObject);
            }
            HasRunLoad = true;
        }
        else
        {
            Debug.LogError("WARNING ... THE AIS HAVE FINALY BECOME SENTIENT (Maybe)... SOMEONE tried to load files multiple times and failed? You should check this before it's too late");
        }
    }
